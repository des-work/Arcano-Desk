...
import { generateTopicPrompt, suggestTopicsFromContent, createTopicFromQuestion, formatGeneratedContent } from '../utils/topicGenerator';
...
export const OllamaProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const generateSummary = useCallback(async (
    ...
    try {
      ...
      console.log(`🧙‍♂️ Wizard's Analysis: ${analysis.reasoning}`);
      console.log(`📊 Content Stats: ${analysis.wordCount} words, complexity: ${analysis.complexityScore}/10`);
      console.log(`🤖 Using Model: ${modelToUse} (${modelInfo?.size || 'unknown size'})`);
...
Magical Summary:`;
      ...
      const summary = response.data.response || 'Failed to generate summary';
...
Magical Answer:`;
      ...
      const answer = response.data.response || 'Failed to generate answer';
  ...
  const generateTopic = useCallback(async (
    ...
    try {
      ...
      const modelInfo = getModelInfo(modelToUse);
      console.log(`🧙‍♂️ Generating ${type} for "${topic}" using ${modelToUse}`);
